...
import com.project.byeoryback.domain.widget.service.WidgetService;
...
public class WidgetController {
    ...
    public ResponseEntity<Widget> createWidget(
        ...
        return ResponseEntity.ok(widgetService.createWidget(userId, request));
    }
     ...
     */
    @GetMapping("/my")
    public ResponseEntity<Page<Widget>> getMyWidgets(
            @RequestHeader("X-User-Id") Long userId,
            @PageableDefault(size = 10, sort = "createdAt", direction = Sort.Direction.DESC) Pageable pageable) {
        return ResponseEntity.ok(widgetService.getMyWidgets(userId, pageable));
    }
     ...
     */
    @GetMapping("/market")
    public ResponseEntity<Page<Widget>> getSharedWidgets(
            @PageableDefault(size = 10, sort = "createdAt", direction = Sort.Direction.DESC) Pageable pageable) {
        return ResponseEntity.ok(widgetService.getSharedWidgets(pageable));
    }
    ...
    public ResponseEntity<Widget> toggleShare(
            @RequestHeader("X-User-Id") Long userId,
            @PathVariable Long id) {
        return ResponseEntity.ok(widgetService.toggleShare(userId, id));
    }
     ...
     */
    @PostMapping("/{id}/fork")
    public ResponseEntity<Widget> forkWidget(
            @RequestHeader("X-User-Id") Long userId,
            @PathVariable("id") Long originalWidgetId) {
        return ResponseEntity.ok(widgetService.forkWidget(userId, originalWidgetId));
    }
}